...
def fetch_weather():
    ...
    try:
        ...
        # Get the current hour (0-23) to use as the starting index
        ...
        current_temp = hourly_temps[current_hour]

        sunrise = data["daily"]["sunrise"][0]
        sunset = data["daily"]["sunset"][0]

        print(f"[WEATHER] Current Temp: {current_temp}°C")
        print(f"[WEATHER] Next 12h -> Rain Volume: {total_rain_mm}mm, Max Probability: {max_rain_prob}%")

        # Format the data into InfluxDB Line Protocol
        line = (
            f"weather_forecast,team={TEAM} "
            f"temp={current_temp},rain_12h={total_rain_mm},rain_prob_12h={max_rain_prob},"
