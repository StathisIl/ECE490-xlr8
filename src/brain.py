...
#  ΟΙ 7  ΚΑΝΟΝΕΣ ΠΟΤΙΣΜΑΤΟΣ (THRESHOLDS) 
...
def is_daytime(sunrise_str: str, sunset_str: str) -> bool:
    ...
    try:
        ...
        now = datetime.now() # Λόγω του os.environ, αυτό είναι ώρα Ελλάδος!
...
def make_decision(moisture, rain_prob, rain_vol, actual_rain, temp, recent_water, sunrise_str, sunset_str):
    """
    Ο  Εγκέφαλος: Ελέγχει και τους 7 κανόνες 
    """
    print("\n" + "="*55)
    ...
    # ΚΑΝΟΝΑΣ 7: Πραγματική Βροχή (Αντί-Ξηρασιακό Fail-Safe)
    elif actual_rain >= ACTUAL_RAIN_THRESHOLD:
        print(" ☔ [STATUS: RULE 7 - RECENT RAIN CONFIRMED]")
        print(f"    ΛΟΓΟΣ: Τις τελευταίες 6 ώρες έπεσαν {actual_rain} mm βροχής.")
...
def main():
    ...
    while True:
        ...
        if None not in (latest_moisture, latest_temp, latest_rain_prob, latest_rain_vol, actual_rain, sunrise, sunset):
            make_decision(latest_moisture, latest_rain_prob, latest_rain_vol, actual_rain, latest_temp, recent_water, sunrise, sunset)
        else:
